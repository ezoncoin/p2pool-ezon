...
from p2pool.ezoncoin import data as ezoncoin_data, helper, height_tracker
...
class P2PNode(p2p.Node):
    ...
    def handle_shares(self, shares, peer):
        ...
        for share, new_txs in shares:
            if new_txs is not None:
                all_new_txs.update((ezoncoin_data.hash256(ezoncoin_data.tx_type.pack(new_tx)), new_tx) for new_tx in new_txs)
    ...
    def handle_bestblock(self, header, peer):
        if self.node.net.PARENT.POW_FUNC(ezoncoin_data.block_header_type.pack(header)) > header['bits'].target:
        ...
        def _(share):
            ...
            def spread():
                if (self.node.get_height_rel_highest(share.header['previous_block']) > -5 or
                    self.node.ezoncoind_work.value['previous_block'] in [share.header['previous_block'], share.header_hash]):
...
class Node(object):
    def __init__(self, factory, ezoncoind, shares, known_verified_share_hashes, net):
        self.factory = factory
        self.ezoncoind = ezoncoind
        self.net = net
    ...
    def start(self):
        ...
        self.ezoncoind_work = variable.Variable((yield helper.getwork(self.ezoncoind, self.net)))
        @defer.inlineCallbacks
        def work_poller():
            while stop_signal.times == 0:
                flag = self.factory.new_block.get_deferred()
                try:
                    self.ezoncoind_work.set((yield helper.getwork(self.ezoncoind, self.net, self.ezoncoind_work.value['use_getblocktemplate'])))
                except:
                    log.err()
        ...
        def handle_header(new_header):
            ...
            if not (self.net.PARENT.POW_FUNC(ezoncoin_data.block_header_type.pack(new_header)) <= self.ezoncoind_work.value['bits'].target):
                return
            ezoncoind_best_block = self.ezoncoind_work.value['previous_block']
            if (self.best_block_header.value is None
                or (
                    new_header['previous_block'] == ezoncoind_best_block and
                    self.net.PARENT.BLOCKHASH_FUNC(ezoncoin_data.block_header_type.pack(self.best_block_header.value)) == ezoncoind_best_block
                ) # new is child of current and previous is current
                or (
                    self.net.PARENT.BLOCKHASH_FUNC(ezoncoin_data.block_header_type.pack(new_header)) == ezoncoind_best_block and
                    self.best_block_header.value['previous_block'] != ezoncoind_best_block
        ...
        def poll_header():
            if self.factory.conn.value is None:
                return
            handle_header((yield self.factory.conn.value.get_block_header(self.ezoncoind_work.value['previous_block'])))
        self.ezoncoind_work.changed.watch(lambda _: poll_header())
        ...
        self.get_height_rel_highest = yield height_tracker.get_height_rel_highest_func(self.ezoncoind, self.factory, lambda: self.ezoncoind_work.value['previous_block'], self.net)
        
        self.best_share_var = variable.Variable(None)
        self.desired_var = variable.Variable(None)
        self.ezoncoind_work.changed.watch(lambda _: self.set_best_share())
        self.set_best_share()
        
        # setup p2p logic and join p2pool network
        
        # update mining_txs according to getwork results
        @self.ezoncoind_work.changed.run_and_watch
        def _(_=None):
            new_mining_txs = {}
            new_known_txs = dict(self.known_txs_var.value)
            for tx_hash, tx in zip(self.ezoncoind_work.value['transaction_hashes'], self.ezoncoind_work.value['transactions']):
                new_mining_txs[tx_hash] = tx
                new_known_txs[tx_hash] = tx
            self.mining_txs_var.set(new_mining_txs)
            self.known_txs_var.set(new_known_txs)
        # add p2p transactions from ezoncoind to known_txs
        @self.factory.new_tx.watch
        def _(tx):
            new_known_txs = dict(self.known_txs_var.value)
            new_known_txs[ezoncoin_data.hash256(ezoncoin_data.tx_type.pack(tx))] = tx
            self.known_txs_var.set(new_known_txs)
        # forward transactions seen to ezoncoind
        ...
        def _(share):
            ...
            if self.ezoncoind_work.value['masternode_payments']:
                block = share.as_block(self.tracker, self.known_txs_var.value, self.ezoncoind_work.value['votes'])
            else:
                block = share.as_block_old(self.tracker, self.known_txs_var.value)

            if block is None:
                print >>sys.stderr, 'GOT INCOMPLETE BLOCK FROM PEER! %s ezoncoin: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
                return
            helper.submit_block(block, True, self.factory, self.ezoncoind, self.ezoncoind_work, self.net)
            print
            print 'GOT BLOCK FROM PEER! Passing to ezoncoind! %s ezoncoin: %s%064x' % (p2pool_data.format_hash(share.hash), self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
            print
    ...
    def set_best_share(self):
        best, desired, decorated_heads, bad_peer_addresses = self.tracker.think(self.get_height_rel_highest, self.ezoncoind_work.value['previous_block'], self.ezoncoind_work.value['bits'], self.known_txs_var.value)
