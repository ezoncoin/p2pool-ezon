...
import p2pool
from ezoncoin import data as ezoncoin_data
...
def get_web_root(wb, datadir_path, ezoncoind_getinfo_var, stop_event=variable.Event()):
    node = wb.node
    ...
    def get_users():
        ...
        for script in sorted(weights, key=lambda s: weights[s]):
            res[ezoncoin_data.script2_to_address(script, node.net.PARENT)] = weights[script]/total_weight
        return res
    ...
    def get_patron_sendmany(total=None, trunc='0.01'):
        ...
        return json.dumps(dict(
            (ezoncoin_data.script2_to_address(script, node.net.PARENT), value/1e8)
            for script, value in get_current_scaled_txouts(total, trunc).iteritems()
            if ezoncoin_data.script2_to_address(script, node.net.PARENT) is not None
        ))
    ...
    def get_global_stats():
        ...
        diff = ezoncoin_data.target_to_difficulty(wb.current_work.value['bits'].target)
        return dict(
            pool_nonstale_hash_rate=nonstale_hash_rate,
            pool_hash_rate=nonstale_hash_rate/(1 - stale_prop),
            pool_stale_prop=stale_prop,
            min_difficulty=ezoncoin_data.target_to_difficulty(node.tracker.items[node.best_share_var.value].max_target),
    ...
    def get_local_stats():
        ...
        my_work = sum(ezoncoin_data.target_to_average_attempts(share.target)
        ...
        for addr in wb.last_work_shares.value:
            miner_last_difficulties[addr] = ezoncoin_data.target_to_difficulty(wb.last_work_shares.value[addr].target)
        ...
        return dict(
            ...
            attempts_to_share=ezoncoin_data.target_to_average_attempts(node.tracker.items[node.best_share_var.value].max_target),
            attempts_to_block=ezoncoin_data.target_to_average_attempts(node.ezoncoind_work.value['bits'].target),
            block_value=node.ezoncoind_work.value['subsidy']*1e-8,
            warnings=p2pool_data.get_warnings(node.tracker, node.best_share_var.value, node.net, ezoncoind_getinfo_var.value, node.ezoncoind_work.value),
    ...
    web_root.putChild('difficulty', WebInterface(lambda: ezoncoin_data.target_to_difficulty(node.tracker.items[node.best_share_var.value].max_target)))
    web_root.putChild('users', WebInterface(get_users))
    web_root.putChild('user_stales', WebInterface(lambda: dict((ezoncoin_data.pubkey_hash_to_address(ph, node.net.PARENT), prop) for ph, prop in
    ...
    web_root.putChild('current_payouts', WebInterface(lambda: dict((ezoncoin_data.script2_to_address(script, node.net.PARENT), value/1e8) for script, value in node.get_current_txouts().iteritems())))
    ...
    web_root.putChild('payout_addr', WebInterface(lambda: ezoncoin_data.pubkey_hash_to_address(wb.my_pubkey_hash, node.net.PARENT)))
    ...
    def update_stat_log():
        ...
        stat_log.append(dict(
            ...
            current_payout=node.get_current_txouts().get(ezoncoin_data.pubkey_hash_to_script2(wb.my_pubkey_hash), 0)*1e-8,
            peers=dict(
                incoming=sum(1 for peer in node.p2p_node.peers.itervalues() if peer.incoming),
                outgoing=sum(1 for peer in node.p2p_node.peers.itervalues() if not peer.incoming),
            ),
    ...
    def get_share(share_hash_str):
        ...
        return dict(
            ...
            share_data=dict(
                ...
                payout_address=ezoncoin_data.script2_to_address(share.new_script, node.net.PARENT),
    ...
    def add_point():
        ...
        hd.datastreams['current_payout'].add_datum(t, current_txouts.get(ezoncoin_data.pubkey_hash_to_script2(wb.my_pubkey_hash), 0)*1e-8)
        miner_hash_rates, miner_dead_hash_rates = wb.get_local_rates()
        current_txouts_by_address = dict((ezoncoin_data.script2_to_address(script, node.net.PARENT), amount) for script, amount in current_txouts.iteritems())
    ...
    x.start(5)
    stop_event.watch(x.stop)
    @node.ezoncoind_work.changed.watch
    def _(new_work):
