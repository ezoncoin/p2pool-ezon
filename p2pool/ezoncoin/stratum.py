...
from p2pool.ezoncoin import data as ezoncoin_data, getwork
...
class StratumRPCMiningProvider(object):
    ...
    def _send_work(self):
        ...
        self.other.svc_mining.rpc_set_difficulty(ezoncoin_data.target_to_difficulty(x['share_target'])*self.wb.net.DUMB_SCRYPT_DIFF).addErrback(lambda err: None)
    ...
    def rpc_submit(self, worker_name, job_id, extranonce2, ntime, nonce):
        ...
        header = dict(
            ...
            merkle_root=ezoncoin_data.check_merkle_link(ezoncoin_data.hash256(new_packed_gentx), x['merkle_link']),
