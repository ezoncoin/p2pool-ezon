...
from p2pool.ezoncoin import data as ezoncoin_data, networks, worker_interface
from p2pool.util import deferral, jsonrpc, math, variable

class ezoncoind(object): # can be used as p2p factory, p2p protocol, or rpc jsonrpc proxy
    def __init__(self):
        ...
        self.headers = {0x16c169477c25421250ec5d32cf9c6d38538b5de970a2355fd89: {
            ...
            'bits': ezoncoin_data.FloatingInteger(bits=0x1a0513c5, target=0x513c50000000000000000000000000000000000000000000000L),
        }}
    ...
    def rpc_getblocktemplate(self, param):
        if param['mode'] == 'template':
            ...
            result = param['data']
            block = ezoncoin_data.block_type.unpack(result.decode('hex'))
            ...
            if block['header']['previous_block'] != self.blocks[-1]:
                return False
            if ezoncoin_data.hash256(result.decode('hex')) > block['header']['bits'].target:
                return False
            header_hash = ezoncoin_data.hash256(ezoncoin_data.block_header_type.pack(block['header']))
        ...
        for i in xrange(100):
            fee = i
            txs.append(dict(
                data=ezoncoin_data.tx_type.pack(dict(version=1, tx_ins=[], tx_outs=[dict(value=fee, script='hello!'*100)], lock_time=0)).encode('hex'),
                fee=fee,
            ))
        return {
            "version" : 2,
...
class MiniNode(object):
    ...
    def start(cls, net, factory, ezoncoind, peer_ports, merged_urls):
        self = cls()
        
        self.n = node.Node(factory, ezoncoind, [], [], net)
        yield self.n.start()
