...
import ezoncoin.p2p as ezoncoin_p2p, ezoncoin.data as ezoncoin_data
from ezoncoin import stratum, worker_interface, helper
...
def main(args, net, datadir_path, merged_urls, worker_endpoint):
    try:
        ...
        @defer.inlineCallbacks
        def connect_p2p():
            # connect to ezoncoind over ezoncoin-p2p
            print '''Testing ezoncoind P2P connection to '%s:%s'...''' % (args.ezoncoind_address, args.ezoncoind_p2p_port)
            factory = ezoncoin_p2p.ClientFactory(net.PARENT)
            reactor.connectTCP(args.ezoncoind_address, args.ezoncoind_p2p_port, factory)
            def long():
                print '''    ...taking a while. Common reasons for this include all of ezoncoind's connection slots being used...'''
        ...
        if args.testnet: # establish p2p connection first if testnet so ezoncoind can work without connections
            factory = yield connect_p2p()
        
        # connect to ezoncoind over JSON-RPC and do initial getmemorypool
        url = '%s://%s:%i/' % ('https' if args.ezoncoind_rpc_ssl else 'http', args.ezoncoind_address, args.ezoncoind_rpc_port)
        print '''Testing ezoncoind RPC connection to '%s' with username '%s'...''' % (url, args.ezoncoind_rpc_username)
        ezoncoind = jsonrpc.HTTPProxy(url, dict(Authorization='Basic ' + base64.b64encode(args.ezoncoind_rpc_username + ':' + args.ezoncoind_rpc_password)), timeout=30)
        yield helper.check(ezoncoind, net)
        temp_work = yield helper.getwork(ezoncoind, net)
        
        ezoncoind_getinfo_var = variable.Variable(None)
        @defer.inlineCallbacks
        def poll_warnings():
            ezoncoind_getinfo_var.set((yield deferral.retry('Error while calling getinfo:')(ezoncoind.rpc_getinfo)()))
        yield poll_warnings()
        ...
        if args.pubkey_hash is None:
            ...
            if address is not None:
                res = yield deferral.retry('Error validating cached address:', 5)(lambda: ezoncoind.rpc_validateaddress(address))()
                if not res['isvalid'] or not res['ismine']:
                    print '    Cached address is either invalid or not controlled by local ezoncoind!'
                    address = None
            
            if address is None:
                print '    Getting payout address from ezoncoind...'
                address = yield deferral.retry('Error getting payout address from ezoncoind:', 5)(lambda: ezoncoind.rpc_getaccountaddress('p2pool'))()
            
            with open(address_path, 'wb') as f:
                f.write(address)
            
            my_pubkey_hash = ezoncoin_data.address_to_pubkey_hash(address, net.PARENT)
        else:
            my_pubkey_hash = args.pubkey_hash
        print '    ...success! Payout address:', ezoncoin_data.pubkey_hash_to_address(my_pubkey_hash, net.PARENT)
        print
        ...
        node = p2pool_node.Node(factory, ezoncoind, shares.values(), known_verified, net)
        yield node.start()
        ...
        web_root = web.get_web_root(wb, datadir_path, ezoncoind_getinfo_var)
                ...
                def signedOn(self):
                    ...
                    def new_share(share):
                        ...
                        if share.pow_hash <= share.header['bits'].target and abs(share.timestamp - time.time()) < 10*60:
                            ...
                            message = '\x02%s BLOCK FOUND by %s! %s%064x' % (net.NAME.upper(), ezoncoin_data.script2_to_address(share.new_script, net.PARENT), net.PARENT.BLOCK_EXPLORER_URL_PREFIX, share.header_hash)
        ...
        def status_thread():
            ...
            while True:
                ...
                try:
                    ...
                    my_shares_per_s = sum(datum['work']/dt/ezoncoin_data.target_to_average_attempts(datum['share_target']) for datum in datums)
                    ...
                    if height > 2:
                        ...
                        this_str += '\n Shares: %i (%i orphan, %i dead) Stale rate: %s Efficiency: %s Current payout: %.4f %s' % (
                            ...
                            node.get_current_txouts().get(ezoncoin_data.pubkey_hash_to_script2(my_pubkey_hash), 0)*1e-8, net.PARENT.SYMBOL,
                        )
                        this_str += '\n Pool: %sH/s Stale rate: %.1f%% Expected time to block: %s' % (
                            math.format(int(real_att_s)),
                            100*stale_prop,
                            math.format_dt(2**256 / node.ezoncoind_work.value['bits'].target / real_att_s),
                        )
                        
                        for warning in p2pool_data.get_warnings(node.tracker, node.best_share_var.value, net, ezoncoind_getinfo_var.value, node.ezoncoind_work.value):
                            print >>sys.stderr, '#'*40
...
def run():
    ...
    parser.add_argument('--net',
        help='use specified network (default: ezoncoin)',
        action='store', choices=sorted(realnets), default='ezoncoin', dest='net_name')
    ...
    parser.add_argument('-a', '--address',
        help='generate payouts to this address (default: <address requested from ezoncoind>)',
    ...
    worker_group.add_argument('-f', '--fee', metavar='FEE_PERCENTAGE',
        help='''charge workers mining to their own ezoncoin address (by setting their miner's username to a ezoncoin address) this percentage fee to mine on your p2pool instance. Amount displayed at http://127.0.0.1:WORKER_PORT/fee (default: 0)''',
        type=float, action='store', default=0, dest='worker_fee')
    
    ezoncoind_group = parser.add_argument_group('ezoncoind interface')
    ezoncoind_group.add_argument('--ezoncoind-address', metavar='DASHD_ADDRESS',
        help='connect to this address (default: 127.0.0.1)',
        type=str, action='store', default='127.0.0.1', dest='ezoncoind_address')
    ezoncoind_group.add_argument('--ezoncoind-rpc-port', metavar='DASHD_RPC_PORT',
        help='''connect to JSON-RPC interface at this port (default: %s <read from ezoncoin.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.RPC_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='ezoncoind_rpc_port')
    ezoncoind_group.add_argument('--ezoncoind-rpc-ssl',
        help='connect to JSON-RPC interface using SSL',
        action='store_true', default=False, dest='ezoncoind_rpc_ssl')
    ezoncoind_group.add_argument('--ezoncoind-p2p-port', metavar='DASHD_P2P_PORT',
        help='''connect to P2P interface at this port (default: %s <read from ezoncoin.conf if password not provided>)''' % ', '.join('%s:%i' % (name, net.PARENT.P2P_PORT) for name, net in sorted(realnets.items())),
        type=int, action='store', default=None, dest='ezoncoind_p2p_port')
    
    ezoncoind_group.add_argument(metavar='DASHD_RPCUSERPASS',
        help='ezoncoind RPC interface username, then password, space-separated (only one being provided will cause the username to default to being empty, and none will cause P2Pool to read them from ezoncoin.conf)',
        type=str, action='store', default=[], nargs='*', dest='ezoncoind_rpc_userpass')
    ...
    if len(args.ezoncoind_rpc_userpass) > 2:
        parser.error('a maximum of two arguments are allowed')
    args.ezoncoind_rpc_username, args.ezoncoind_rpc_password = ([None, None] + args.ezoncoind_rpc_userpass)[-2:]
    
    if args.ezoncoind_rpc_password is None:
        conf_path = net.PARENT.CONF_FILE_FUNC()
        if not os.path.exists(conf_path):
            parser.error('''Ezoncoin configuration file not found. Manually enter your RPC password.\r\n'''
                '''If you actually haven't created a configuration file, you should create one at %s with the text:\r\n'''
                '''\r\n'''
                '''server=1\r\n'''
                '''rpcpassword=%x\r\n'''
                '''\r\n'''
                '''Keep that password secret! After creating the file, restart Ezoncoin.''' % (conf_path, random.randrange(2**128)))
        ...
        for conf_name, var_name, var_type in [
            ('rpcuser', 'ezoncoind_rpc_username', str),
            ('rpcpassword', 'ezoncoind_rpc_password', str),
            ('rpcport', 'ezoncoind_rpc_port', int),
            ('port', 'ezoncoind_p2p_port', int),
        ]:
            if getattr(args, var_name) is None and conf_name in contents:
                setattr(args, var_name, var_type(contents[conf_name]))
        if args.ezoncoind_rpc_password is None:
            parser.error('''Ezoncoin configuration file didn't contain an rpcpassword= line! Add one!''')
    
    if args.ezoncoind_rpc_username is None:
        args.ezoncoind_rpc_username = ''
    
    if args.ezoncoind_rpc_port is None:
        args.ezoncoind_rpc_port = net.PARENT.RPC_PORT
    
    if args.ezoncoind_p2p_port is None:
        args.ezoncoind_p2p_port = net.PARENT.P2P_PORT
    ...
    if args.address is not None:
        try:
            args.pubkey_hash = ezoncoin_data.address_to_pubkey_hash(args.address, net.PARENT)
        except Exception, e:
